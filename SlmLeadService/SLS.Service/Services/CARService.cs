...
namespace SLS.Service.Services
{
    public class CARService
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(CARService));

        public static bool CreateActivityLog(CARServiceData data, string ticketId)
        {
            string errMsg = "";
            try
            {
                CARServiceProxy.LogServiceHeader header = new CARServiceProxy.LogServiceHeader()
                {
                    ...
                    ServiceName = ApplicationConstant.CARLogService.CARCreateActivityLogServiceName,
                    SystemCode = data.SystemCode,
                    TransactionDateTime = data.TransactionDateTime
                };

                data.ServiceName = ApplicationConstant.CARLogService.CARCreateActivityLogServiceName;
                ...
                service.InnerChannel.OperationTimeout = new TimeSpan(0, 0, ApplicationConstant.CARLogService.CARTimeout);
                ...
                {
                    string message = "TicketId=" + ticketId + ", Call CAR WS Failure = " + result.ResponseStatus.ResponseCode + ": " + result.ResponseStatus.ResponseMessage;
                    log.Error(message);
                   
                    if (!AddCARLog(LogType.Error, result.ResponseStatus.ResponseCode, result.ResponseStatus.ResponseMessage, data, out errMsg))
                        log.Error("Insert kkslm_tr_cas_resend failed: " + errMsg);

                    return false;

                    //AddCARBatch(data, data.SystemCode, out errMsg, ticketId);
                }
            }
            catch (Exception ex)
            {
                string message = ex.InnerException != null ? ex.InnerException.Message : ex.Message;
                message = "TicketId=" + ticketId + ", Call CAR WS Failure = " + message;
                log.Error(message);
        ...
        //        {
        //            running = ApplicationUtil.SafeInt(tmp[1]);
        //            // check file exists
        ...
        //public static bool AddCARBatch(CARServiceData data, string systemCode, out string error, string ticketId)
        //{
        ...
        //        // default
        //        string folder = ApplicationConstant.CARLogService.CARBatchFolder;
        //        if (folder == "")
        //        {
        //            log.Error("ไม่พบ Folder สำหรับจัดเก็บไฟล์ Excel เพื่อซ่อมการยิง WS CAR");
        //            return false;
        //        }
        ...
        //            log.Error("ไม่สามารถสร้างไฟล์ xml เพื่อซ่อมการยิง WS CAR : " + error);
        //            ret = false;
        //        }
        ...
        public static bool AddCARLog(LogType _logType, string ResponseCode, string ErrorMessage, CARServiceData data, out string error)
        {
            ...
            {
                // get config
                string errorCodeResend = ApplicationConstant.CARLogService.CARErrorCodeResend;
                int maxResend = ApplicationConstant.CARLogService.CARMaxResend;
